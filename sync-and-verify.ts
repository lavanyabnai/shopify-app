/**
 * Sync and Verify Script
 *
 * Syncs orders from Shopify to local database and generates
 * initial inventory snapshots for War Room testing
 *
 * Usage: npx tsx sync-and-verify.ts
 */
...
async function fetchOrdersFromShopify(admin: any, shop: string, since?: Date) {
...
async function fetchProductsFromShopify(admin: any, shop: string) {
...
async function main() {
  ...
  // Get session to access Shopify admin API
  ...
  try {
    ...
    // Fetch orders from last 30 days (including new Oct 2025 orders)
    const since = new Date('2025-10-01');
    const orders = await fetchOrdersFromShopify(admin, shop, since);
    ...
    const products = await fetchProductsFromShopify(admin, shop);
